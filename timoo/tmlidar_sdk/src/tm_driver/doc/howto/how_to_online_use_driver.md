...
- ring ------ The ring ID of the point, which represents the row number. e.g. For TM80, the range of ring ID is 0~79 (from bottom to top).
...
{
  TM_WARNING << "Error code : " << code.toString() << TM_REND;
}
```
...
### 2.5 Define the parameter and configure the parameter
 ...
 The msop port and difop port number of lidar can be got from wireshark(a network socket capture software). The default value is ```msop-2368``` and ```difop-8603```. User also need to make sure the ```lidar_type``` is set correctly.

```c++
TMDriverParam param;                      		  ///< Create a parameter object
param.input_param.msop_port = 2368;              ///< Set the lidar msop port number the default 2368
param.input_param.difop_port = 8603;             ///< Set the lidar difop port number the default 8603
...
In tm_driver, the point cloud is stored in **column major order**, which means if there is  a point msg.point_cloud_ptr->at(i) , the next point on the same ring should be msg.point_cloud_ptr->at(i+msg.height). User can set the parameter ```saved_by_rows``` to ```true``` to make the point cloud stored in **row major order**.



### *Congratulations! You have finished the demo tutorial of Timoo LiDAR driver! You can find the complete demo code in the demo folder under the project directory. Feel free to connect us if you have any question about the driver.*