#pragma once
#include <tm_driver/msg/point_cloud_msg.h>
#include <tm_driver/msg/packet_msg.h>
#include <tm_driver/msg/scan_msg.h>
#include <tm_driver/msg/imu_msg.h>
#include <tm_driver/utility/lock_queue.h>
#include <tm_driver/utility/thread_pool.hpp>
#include <tm_driver/utility/time.h>
#include <tm_driver/common/error_code.h>
#include <tm_driver/driver/input.hpp>
#include <tm_driver/driver/decoder/decoder_factory.hpp>
...
public:
  LidarDriverImpl();
  ~LidarDriverImpl();
  bool init(const TMDriverParam& param);
  void initDecoderOnly(const TMDriverParam& param);
  bool start();
  void stop();
...
private:
  ...
  void setImuMsgHeader(ImuMsg& msg);
...
private:
  ...
  //申明IMUMSG
  ImuMsg::ImuVecPtr imu_ptr_;
};

template <typename T_Point>
inline LidarDriverImpl<T_Point>::LidarDriverImpl()
  : init_flag_(false), start_flag_(false), difop_flag_(false), point_cloud_seq_(0), imu_seq_(0), scan_seq_(0), ndifop_count_(0)
{
  thread_pool_ptr_ = std::make_shared<ThreadPool>();
  point_cloud_ptr_ = std::make_shared<typename PointCloudMsg<T_Point>::PointCloud>();
  imu_ptr_ = std::make_shared<ImuMsg::ImuVec>();
...
template <typename T_Point>
inline bool LidarDriverImpl<T_Point>::init(const TMDriverParam& param)
{
  if (init_flag_)
  {
    return false;
...
template <typename T_Point>
inline void LidarDriverImpl<T_Point>::initDecoderOnly(const TMDriverParam& param)
{
  if (init_flag_)
  {
    return;
...
template <typename T_Point>
inline void LidarDriverImpl<T_Point>::regRecvCallback(const std::function<void(const ImuMsg&)>& callback)
{
...
inline bool LidarDriverImpl<T_Point>::decodeMsopScan(const ScanMsg& scan_msg, PointCloudMsg<T_Point>& point_cloud_msg)
{
  ...
  for (int i = 0; i < static_cast<int>(scan_msg.packets.size()); i++)
  {
    ...
    switch (ret)
    {
      case TMDecoderResult::DECODE_OK:
      case TMDecoderResult::FRAME_SPLIT:
        pointcloud_one_frame[i] = std::move(pointcloud_one_packet);
        break;
      case TMDecoderResult::WRONG_PKT_HEADER:
        reportError(Error(ERRCODE_WRONGPKTHEADER));
        break;
      case TMDecoderResult::PKT_NULL:
...
template <typename T_Point>
inline void LidarDriverImpl<T_Point>::decodeDifopPkt(const PacketMsg& msg,std::vector<TmImu>& imu_vec)
{
  lidar_decoder_ptr_->processDifopPkt(msg.packet.data(),imu_vec);
  difop_flag_ = true;
}
...
template <typename T_Point>
inline void LidarDriverImpl<T_Point>::runCallBack(const ImuMsg& msg)
{
  if (msg.seq != 0)
  {
    for (auto& it : imu_cb_vec_)
    {
      it(msg);
    }
  }
...
inline void LidarDriverImpl<T_Point>::processDifop()
{
  while (difop_pkt_queue_.size() > 0)
  {
    ...
    if(!driver_param_.decoder_param.use_imu){
      decodeDifopPkt(pkt);
      runCallBack(pkt);
    }else{
      decodeDifopPkt(pkt,*imu_ptr_);
      ImuMsg msg(imu_ptr_);
      setImuMsgHeader(msg);;
      if (driver_param_.decoder_param.use_lidar_clock == true)
      {
        msg.imu_timestamp = lidar_decoder_ptr_->getIMUTime(pkt.packet.data());
      }
      else
      {
        msg.imu_timestamp = getTime();
      }
      if (imu_ptr_->size() == 0)
      {
        reportError(Error(ERRCODE_ZEROIMU));
      }
      else
      {
        runCallBack(msg);
      }
      imu_ptr_.reset(new ImuMsg::ImuVec);
    }
  }
...
template <typename T_Point>
inline void LidarDriverImpl<T_Point>::setImuMsgHeader(ImuMsg& msg)
{
  msg.seq = imu_seq_++;
  msg.frame_id = driver_param_.frame_id;
}
