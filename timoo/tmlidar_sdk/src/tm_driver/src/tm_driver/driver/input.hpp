...
constexpr double TM16_PCAP_SLEEP_DURATION = 1200;  ///< us
...
{
public:
  Input(const LidarType& type, const TMInputParam& input_param, const std::function<void(const Error&)>& excb);
  ~Input();
  bool init();
  bool start();
  void stop();
...
inline Input::Input(const LidarType& type, const TMInputParam& input_param,
...
inline void Input::getPcapPacket()
{
  while (pcap_thread_.start_.load())
  {
    ...
    {
      case LidarType::TM16:
        time2go += std::chrono::microseconds(static_cast<long long>(TM16_PCAP_SLEEP_DURATION / input_param_.pcap_rate));
        break;
      case LidarType::TM64:
        time2go += std::chrono::microseconds(static_cast<long long>(TM16_PCAP_SLEEP_DURATION / input_param_.pcap_rate));
        break;
