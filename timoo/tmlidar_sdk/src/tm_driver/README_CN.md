...
## 1 工程简介
  ...
  **tm_driver**为天眸光电跨平台的雷达驱动内核，方便用户二次开发使用。
...
**tm_driver** 依赖下列的第三方库，在编译之前需要先安装的它们：
...
*注：在Windows中，**tm_driver** 暂不支持安装使用。*
...
使用时，需要在```CMakeLists```文件中使用find_package()指令找到**tm_driver**，然后链接相关库。

```cmake
find_package(tm_driver REQUIRED)
include_directories(${tm_driver_INCLUDE_DIRS})
target_link_libraries(project ${tm_driver_LIBRARIES})
```

### 3.2 作为子模块使用

在```CMakeLists```文件中将**tm_driver**作为子模块添加到工程内，使用find_package()指令找到**tm_driver**，然后链接相关库。

```cmake
add_subdirectory(${PROJECT_SOURCE_DIR}/tm_driver)
find_package(tm_driver REQUIRED)
include_directories(${tm_driver_INCLUDE_DIRS})
target_link_libraries(project ${tm_driver_LIBRARIES})
```
...
**tm_driver**提供了两个示例程序，存放于```tm_driver/demo``` 中：
...
**tm_driver**提供了一个基于PCL的点云可视化工具，存放于```tm_driver/tool``` 中：

- tm_driver_viewer.cpp
...
具体使用请参考[可视化工具操作指南](doc/howto/how_to_use_tm_driver_viewer.md) 



## 6 坐标变换

 **tm_driver**提供了内置的坐标变换功能，可以直接输出经过坐标变换后的点云，节省了用户对点云进行坐标变换的额外操作耗时。若希望启用此功能，执行CMake配置时加上参数：
...
- 参数定义: ```tm_driver/src/tm_driver/driver/driver_param.h```
- 点云消息定义: ```tm_driver/src/tm_driver/msg/point_cloud_msg.h```
- 接口定义: ```tm_driver/src/tm_driver/api/lidar_driver.h```
- 错误码定义: ```tm_driver/src/tm_driver/common/error_code.h```
