...
{
public:
  typedef typename std::shared_ptr<CTMEndian> Ptr;
  typedef typename std::shared_ptr<const CTMEndian> ConstPtr;
...
public:
  CTMEndian()
  {
    ...
    {
      host_endian_type_ = DataEndianType::TM_BIG_ENDIAN;
    }
    else
    {
      host_endian_type_ = DataEndianType::TM_SMALL_ENDIAN;
    }
  }
...
struct alignas(16) ProtoMsgHeader
{
  ...
  bool toTargetEndianArray(char* p_array, const unsigned int maxArraySize,
                           const DataEndianType dst_endian = DataEndianType::TM_BIG_ENDIAN) const
  {
    ...
    CTMEndian<unsigned int> uint32Endian;
    CTMEndian<std::time_t> uint64Endian;
  ...
  bool toHostEndianValue(const char* p_array, const unsigned int arraySize,
                         const DataEndianType src_endian = DataEndianType::TM_BIG_ENDIAN)
  {
...
public:
  ...
  inline int initSender(std::string proto_send_port, std::string proto_send_ip)
  {
    ...
    {
      TM_ERROR << "Proto Sender Port is already used! Abort!" << TM_REND;
      exit(-1);
    }
    return 0;
  }
  ...
  inline int initReceiver(uint16_t proto_recv_port)
  {
    ...
    {
      TM_ERROR << "Proto Receiver Port is already used! Abort!" << TM_REND;
      exit(-1);
    }
  ...
  inline int sendProtoMsg(const void* pMsgData, const ProtoMsgHeader& header)
  {
    ...
    header.toTargetEndianArray(sendBuffer, sizeof(ProtoMsgHeader), DataEndianType::TM_BIG_ENDIAN);
  ...
  inline int receiveProtoMsg(void* pMsgData, const int msgMaxLen, ProtoMsgHeader& header)
  {
    ...
    header.toHostEndianValue(pRecvBuffer, sizeof(ProtoMsgHeader), DataEndianType::TM_BIG_ENDIAN);
