...
## 1 工程简介
 **tmlidar_sdk** 为天眸光电在Ubuntu环境下的雷达驱动软件包，包括了雷达驱动内核， ROS拓展功能，ROS2拓展功能，Protobuf-UDP通信拓展功能。对于没有二次开发需求的用户，或是想直接使用ROS或ROS2进行二次开发的用户，可直接使用本软件包， 配合ROS或ROS2自带的RVIZ可视化工具即可查看点云。 对于有更深一步二次开发需求，想将雷达驱动集成到自己工程内的客户， 请参考雷达驱动内核的相关文档，直接使用内核[tm_driver](https://github.com/Timoo-LiDAR/tm_driver)进行二次开发。
...
### 2.1 使用git clone
 ...
 由于tmlidar_sdk项目中包含子模块驱动内核tm_driver, 因此在执行git clone 后还需要执行相关指令初始化并更新子模块。

  ```sh
git clone https://github.com/Timoo-LiDAR/tmlidar_sdk.git
cd tmlidar_sdk
git submodule init
...
用户可以直接访问  [tmlidar_sdk_release](https://github.com/Timoo-LiDAR/tmlidar_sdk/releases) 下载最新版本的tmlidar_sdk. 请下载 **tmlidar_sdk.tar.gz** 压缩包， 不要下载Source code。 因为Source code压缩包内不包含子模块tm_driver的代码， 用户还需自行下载tm_driver的代码放入其中才行。
...
(2) 将tmlidar_sdk工程目录下的*package_ros1.xml*文件重命名为*package.xml*。

(3) 新建一个文件夹作为工作空间，然后再新建一个名为*src*的文件夹, 将tmlidar_sdk工程放入*src*文件夹内。
...
roslaunch tmlidar_sdk start.launch
```
...
(2) 将tmlidar_sdk工程目录下的*package_ros2.xml*文件重命名为*package.xml*。

(3) 新建一个文件夹作为工作空间，然后再新建一个名为*src*的文件夹, 将tmlidar_sdk工程放入*src*文件夹内。

(4) 通过[链接](https://github.com/Timoo-LiDAR/tmlidar_msg)下载ROS2环境下的雷达packet消息定义， 将tmlidar_msg工程也放在刚刚新建的*src*文件夹内，与tmlidar_sdk并列。
...
ros2 launch tmlidar_sdk start.py
```
