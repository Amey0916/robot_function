...
class GestureBodyDetectionNode(Node):
    def __init__(self):
        ...
        # 新增：come模式画面接收相关
        self.come_frame = None  # 存储come模式的画面
        self.come_frame_lock = threading.Lock()  # come画面互斥锁
        ...
        # 新增：订阅come模式的画面话题
        self.come_frame_sub = self.create_subscription(
            Image,
            '/come_mode_frame',  # come_mode_node发布的画面话题
            self.come_frame_callback,
            10
        )
        ...
        self.get_logger().info("📺 已订阅come模式画面话题：/come_mode_frame（唯一显示窗口整合）")
    ...
    def come_mode_cb(self, msg):
        ...
        # 核心新增：come激活时，强制关闭follow模式并发布状态
        if self.come_mode_active:
            if self.is_follow_mode:
                self.is_follow_mode = False
                self.follow_gesture_count = 0
                self._publish_follow_mode_active(False)
                self._publish_stop_orientation()
                self.get_logger().warn("🚫 come模式已激活，强制关闭follow模式！")

    # 新增：接收come模式画面的回调函数
    def come_frame_callback(self, msg):
        """接收come_mode_node发布的画面并缓存"""
        try:
            with self.come_frame_lock:
                self.come_frame = self.bridge.imgmsg_to_cv2(msg, encoding="bgr8")
        except Exception as e:
            self.get_logger().warn(f"接收come画面失败：{e}")
    ...
    def voice_follow_callback(self, msg):
        ...
        if now - self.last_follow_time >= FOLLOW_DEBOUNCE_SECONDS:
            if cmd == "start_follow":
                # ===================== 核心修复 =====================
                # come 模式激活时，绝对禁止开启 follow
                if self.come_mode_active:
                    self.get_logger().warn("🚫 互斥拒绝：当前处于 come 模式，无法激活 follow 模式（语音触发）！")
                    return
                # ===================================================
    ...
    def display_loop(self):
        """显示线程：唯一显示窗口，优先显示come画面，否则显示检测画面"""
        # 确保只创建一个窗口
        cv2.namedWindow("Gesture + Body + Come Detection (Q to quit)", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("Gesture + Body + Come Detection (Q to quit)", CAMERA_WIDTH, CAMERA_HEIGHT)
        while not self.stop_event.is_set():
            display_frame = None
            
            # 优先使用come模式的画面（如果有且come模式激活）
            with self.come_frame_lock:
                if self.come_mode_active and self.come_frame is not None:
                    display_frame = self.come_frame.copy()
            
            # 否则使用检测画面
            if display_frame is None:
                with self.frame_lock:
                    if self.latest_display_frame is not None:
                        display_frame = self.latest_display_frame.copy()
                    elif self.latest_frame is not None:
                        display_frame = self.latest_frame.copy()
            
            # 显示画面
            if display_frame is not None:
                cv2.imshow("Gesture + Body + Come Detection (Q to quit)", display_frame)
        ...
        # 关闭唯一窗口
        cv2.destroyWindow("Gesture + Body + Come Detection (Q to quit)")

    def detect_loop(self):
        """核心检测循环：基于最新帧推理，不阻塞采集与显示"""
        # 核心新增：come激活时直接返回，彻底关闭follow相关检测
        if self.come_mode_active:
            # 确保follow模式被强制关闭
            if self.is_follow_mode:
                self.is_follow_mode = False
            ...
            for hand_landmarks in hand_detection_result.hand_landmarks:
                ...
                if is_palm_open:
                    ...
                    if index_straight and middle_straight and ring_bent and pinky_bent and index_ok and 20 < index_middle_angle < 60:
                        # follow 模式下禁止开启 come
                        if self.is_follow_mode:
                            self.get_logger().warn("🚫 互斥拒绝：当前处于 follow 模式，无法触发 come 手势！")
                        else:
        ...
        if current_gesture == "follow":
            ...
            if now - self.last_follow_time >= FOLLOW_DEBOUNCE_SECONDS:
                # ===================== 核心修复 =====================
                # come 模式激活时，绝对禁止开启 follow（只允许关闭）
                if self.come_mode_active:
                    self.get_logger().warn("🚫 互斥拒绝：当前处于 come 模式，无法激活 follow 模式（手势触发）！")
                    return
                # ===================================================
