                                      ...
                                      const AS_SDK_CAM_MODEL_E &cam_type)
{
    ...
    case AS_SDK_CAM_MODEL_NUWA_HP60V:
        stPublisherInfo.depth_camera_info_pub =
            this->create_publisher<sensor_msgs::msg::CameraInfo>("~/depth/camera_info", 5);
        stPublisherInfo.depth_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/depth/image_raw" +
                                        std::to_string(dev_idx), 100);
        stPublisherInfo.ir_camera_info_pub = this->create_publisher<sensor_msgs::msg::CameraInfo>("~/ir/camera_info",
                                             5);
        stPublisherInfo.ir_img_pub = this->create_publisher<sensor_msgs::msg::Image>("~/ir/image" + std::to_string(
                                         dev_idx), 100);
        stPublisherInfo.depth_points_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("~/depth/points" +
                                           std::to_string(dev_idx), 100);
        break;
    ...
    case AS_SDK_CAM_MODEL_VEGA:
        stPublisherInfo.depth_camera_info_pub =
            this->create_publisher<sensor_msgs::msg::CameraInfo>("~/depth/camera_info", 5);
        stPublisherInfo.depth_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/depth/image_raw", rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(rmw_qos_profile_default),rmw_qos_profile_default));
        stPublisherInfo.rgb_camera_info_pub = this->create_publisher<sensor_msgs::msg::CameraInfo>("~/rgb/camera_info",5);
        stPublisherInfo.rgb_img_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/rgb/image", 5);
        stPublisherInfo.depth_points_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("~/depth/points",rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(rmw_qos_profile_default),
                                    rmw_qos_profile_default));
        break;
    case AS_SDK_CAM_MODEL_KUNLUN_A:
        stPublisherInfo.depth_camera_info_pub =
            this->create_publisher<sensor_msgs::msg::CameraInfo>("~/depth/camera_info", 5);
        stPublisherInfo.depth_odd_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/depth/odd_image_raw" +
                                            std::to_string(dev_idx), 100);
        stPublisherInfo.depth_even_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/depth/even_image_raw" +
                                             std::to_string(dev_idx), 100);
        stPublisherInfo.depth_merge_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/depth/merge_image_raw"
                                              + std::to_string(dev_idx), 100);
        stPublisherInfo.peak_camera_info_pub =
            this->create_publisher<sensor_msgs::msg::CameraInfo>("~/peak/camera_info", 5);
        stPublisherInfo.peak_odd_img_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/peak/odd_image" +
                                               std::to_string(dev_idx), 100);
        stPublisherInfo.peak_even_img_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/peak/even_image" +
                                                std::to_string(dev_idx), 100);
        stPublisherInfo.depth_odd_points_pub =
            this->create_publisher<sensor_msgs::msg::PointCloud2>("~/depth/odd_points" +
                    std::to_string(dev_idx), 100);
        stPublisherInfo.depth_even_points_pub =
            this->create_publisher<sensor_msgs::msg::PointCloud2>("~/depth/even_points" +
                    std::to_string(dev_idx), 100);
        stPublisherInfo.depth_merge_points_pub =
            this->create_publisher<sensor_msgs::msg::PointCloud2>("~/depth/merge_points" +
                    std::to_string(dev_idx), 100);
        break;
    case AS_SDK_CAM_MODEL_KUNLUN_C:
        ...
        stPublisherInfo.depth_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/depth/image_raw" +
                                        std::to_string(dev_idx), 100);
        stPublisherInfo.rgb_camera_info_pub = this->create_publisher<sensor_msgs::msg::CameraInfo>("~/rgb/camera_info",
                                              5);
        stPublisherInfo.rgb_img_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/rgb/image" +
                                          std::to_string(dev_idx), 100);
        stPublisherInfo.peak_camera_info_pub =
            this->create_publisher<sensor_msgs::msg::CameraInfo>("~/peak/camera_info", 5);
        stPublisherInfo.peak_img_raw_pub = this->create_publisher<sensor_msgs::msg::Image>("~/peak/image" +
                                           std::to_string(dev_idx), 100);
        stPublisherInfo.depth_points_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("~/depth/points" +
                                           std::to_string(dev_idx), 100);
        break;
...
int CameraPublisher::onCameraStart(AS_CAM_PTR pCamera)
{
    ...
    // int retry = 0;
    // AS_CAM_Parameter_s stIrRgbParameter;
    // memset(&stIrRgbParameter, 0, sizeof(AS_CAM_Parameter_s));
    // while(retry++ < 20) {
    //     ret = AS_SDK_GetCamParameter(pCamera, &stIrRgbParameter);
    //     // if (ret != 0) {
    //     //     LOG(ERROR) << "get camera parameter failed" << std::endl;
    //     // }
    //     std::this_thread::sleep_for(std::chrono::milliseconds(100));
    // }
    // if (m_cam_parameter.find(pCamera) == m_cam_parameter.end()) {
    //     m_cam_parameter.insert(std::make_pair(pCamera, stIrRgbParameter));
    // }
    return ret;
}
        ...
        AS_CAM_Parameter_s &stParameter, builtin_interfaces::msg::Time time)
{
    ...
    if (cam_type != AS_SDK_CAM_MODEL_KONDYOR) {
        genRosDepthCamInfo(&pstData->depthImg, &stDepthCamInfo, stParameter);
    }
    ...
    if (m_launch_param.color_pcl == false) {
        ...
        if (rclcpp::ok()) {
            for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
                if (it->pCamera == pCamera) {
                    if (it->config_info.is_Registration) {
                        pointCloudMsg.header.frame_id = std::string(this->get_name());//  + "_color" + std::to_string(it->index);
                    } else {
                        pointCloudMsg.header.frame_id = std::string(this->get_name());// + "_depth" + std::to_string(it->index);
                    }
                    if (cam_type == AS_SDK_CAM_MODEL_KUNLUN_A) {
                        if (pData->pointCloud.height == 96) {
                            it->depth_odd_points_pub->publish(pointCloudMsg);
                        } else if (pData->pointCloud.height == 8) {
                            it->depth_even_points_pub->publish(pointCloudMsg);
                        }
                    } else {
        ...
        for (unsigned int y = 0; y < pData->depthImg.height; y++) {
            for (unsigned int x = 0; x < pData->depthImg.width; x++) {
                unsigned short depth = ((unsigned short *)pData->depthImg.data)[y * pData->depthImg.width + x];
                if (depth == 0)
                    continue;
                pcl::PointXYZRGB p;
                                        ...
                                        AS_CAM_Parameter_s &stParameter, builtin_interfaces::msg::Time time)
{
    ...
    if (rclcpp::ok()) {
        for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
            if (it->pCamera == pCamera) {
                stPeakCamInfo.header.frame_id = "depth" + std::to_string(it->index);
                peakImage.header.frame_id = "depth" + std::to_string(it->index);
                it->peak_camera_info_pub->publish(stPeakCamInfo);
                if (cam_type == AS_SDK_CAM_MODEL_KUNLUN_A) {
                    if (pstData->peakImg.height == 96) {
                        it->peak_odd_img_raw_pub->publish(peakImage);
                    } else if (pstData->peakImg.height == 8) {
                        it->peak_even_img_raw_pub->publish(peakImage);
                    }
                } else {
                    it->peak_img_raw_pub->publish(peakImage);
                }
            }
        }
    }
        ...
        AS_CAM_Parameter_s &stParameter)
{
    ...
    for (int i = 0 ; i < stPointCloud.points.size(); i++) {
        int index = i * 3;
    ...
    if (rclcpp::ok()) {
        for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
            if (it->pCamera == pCamera) {
                if (it->config_info.is_Registration) {
                    pointCloudMsg.header.frame_id = "color" + std::to_string(it->index);
                } else {
                    pointCloudMsg.header.frame_id = "depth" + std::to_string(it->index);
                }
                it->depth_merge_points_pub->publish(pointCloudMsg);
            }
        }
    }
}
        ...
        if (it->pCamera == pCamera) {
            ...
            if (it_par != m_cam_type_map.end()) {
                if ((it_par->second == AS_SDK_CAM_MODEL_HP60C) || (it_par->second == AS_SDK_CAM_MODEL_HP60CN)
                    ...
                    // geometry_msgs::TransformStamped cameraLinkTransform;
                    // cameraLinkTransform.header.stamp = this->get_clock()->now();
                    // cameraLinkTransform.header.frame_id = "world";
                    // cameraLinkTransform.child_frame_id = "camera_link";
                    // cameraLinkTransform.transform.translation.x = 1.0;
                    // cameraLinkTransform.transform.rotation.w = 1.0;

                    // br.sendTransform(cameraLinkTransform);

                    /* [TODO] if multi camera connetecd, publish their tf respective.
                    one ros2 node operate one camera or one ros2 node operate multi camera with different frame_id name.
                    */

                    geometry_msgs::msg::TransformStamped rgbTransform;
                    rgbTransform.header.stamp = this->get_clock()->now();
                    rgbTransform.header.frame_id = std::string(this->get_name()) + "_camera_link";//"camera_link" + std::to_string(it->index);
                    rgbTransform.child_frame_id = std::string(this->get_name()) + "_color";//"color" + std::to_string(it->index);
...
int CameraPublisher::initLaunchParams()
{
    ...
    this->get_parameter_or<double>("pitch", pitch_, 0.0);

    this->declare_parameter<double>("roll", 0.0);
    this->get_parameter_or<double>("roll", roll_, 0.0);

    this->declare_parameter<double>("yaw", 0.0);
    this->get_parameter_or<double>("yaw", yaw_, 0.0);
