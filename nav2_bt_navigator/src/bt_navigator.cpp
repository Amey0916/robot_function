...
BtNavigator::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  record_path_navigator_ = std::make_unique<nav2_bt_navigator::NavigateFollowRecordPathNavigator>();
  ...
  if (!record_path_navigator_->on_configure(
      shared_from_this(), plugin_lib_names, feedback_utils, &plugin_muxer_, odom_smoother_))
  {
    return nav2_util::CallbackReturn::FAILURE;
  }
...
BtNavigator::on_activate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  if (!record_path_navigator_->on_activate() || !poses_navigator_->on_activate() || !pose_navigator_->on_activate()) {
    return nav2_util::CallbackReturn::FAILURE;
  }
...
BtNavigator::on_deactivate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  if (!poses_navigator_->on_deactivate() || !pose_navigator_->on_deactivate() || !record_path_navigator_->on_deactivate()) {
    return nav2_util::CallbackReturn::FAILURE;
  }
...
BtNavigator::on_cleanup(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  if (!poses_navigator_->on_cleanup() || !pose_navigator_->on_cleanup() || !record_path_navigator_->on_cleanup()) {
    return nav2_util::CallbackReturn::FAILURE;
  }

  poses_navigator_.reset();
  pose_navigator_.reset();
  record_path_navigator_.reset();
