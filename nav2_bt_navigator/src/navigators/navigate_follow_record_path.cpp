...
#include <memory>
#include <limits>
#include "nav2_bt_navigator/navigators/navigate_follow_record_path.hpp"

namespace nav2_bt_navigator
{

bool
NavigateFollowRecordPathNavigator::configure(
  ...
  std::shared_ptr<nav2_util::OdomSmoother> odom_smoother)
{
  ...
  goal_sub_ = node->create_subscription<nav_msgs::msg::Path>(
    "record_path",
    rclcpp::SystemDefaultsQoS(),
    std::bind(&NavigateFollowRecordPathNavigator::onGoalPoseReceived, this, std::placeholders::_1));

  return true;
}

std::string
NavigateFollowRecordPathNavigator::getDefaultBTFilepath(
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  std::string default_bt_xml_filename;
  auto node = parent_node.lock();

  if (!node->has_parameter("default_follow_record_path_bt_xml")) {
    std::string pkg_share_dir =
      ament_index_cpp::get_package_share_directory("nav2_bt_navigator");
    node->declare_parameter<std::string>(
      "default_follow_record_path_bt_xml",
      pkg_share_dir +
      "/behavior_trees/navigate_follow_record_path_w_replanning_and_recovery.xml");
  }

  node->get_parameter("default_follow_record_path_bt_xml", default_bt_xml_filename);

  return default_bt_xml_filename;
}

bool
NavigateFollowRecordPathNavigator::cleanup()
{
  goal_sub_.reset();
  self_client_.reset();
  return true;
...
bool
NavigateFollowRecordPathNavigator::goalReceived(ActionT::Goal::ConstSharedPtr goal)
{
  auto bt_xml_filename = goal->behavior_tree;

  std::cout << "goalReceived PPPPPPPPPPPPPPP " << bt_xml_filename << std::endl;
  ...
  bt_action_server_->loadBehaviorTree11();

  return true;
}

void
NavigateFollowRecordPathNavigator::goalCompleted(
...
void
NavigateFollowRecordPathNavigator::onLoop()
{
...
void
NavigateFollowRecordPathNavigator::onPreempt(ActionT::Goal::ConstSharedPtr goal)
{
...
void
NavigateFollowRecordPathNavigator::initializeGoalPose(ActionT::Goal::ConstSharedPtr goal)
{
  RCLCPP_INFO(
...
void
NavigateFollowRecordPathNavigator::onGoalPoseReceived(const nav_msgs::msg::Path::SharedPtr path)
{
    RCLCPP_INFO(
    logger_, "Received record path,begin to follow path.");
